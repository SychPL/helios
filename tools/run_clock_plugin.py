...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path!='/plugin.dex' or self.client_address[0]!='192.168.1.113':self.send_error(403);return
        self.send_response(200);self.send_header('Content-Length',str(len(payload)));self.end_headers();self.wfile.write(payload)
    def log_message(self,*a):pass
with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as sock:sock.connect(('192.168.1.113',8555));host=sock.getsockname()[0]
...
try:
    ...
    result=urllib.request.urlopen('http://192.168.1.113:8555/agent/dex?'+urllib.parse.urlencode(params),timeout=45).read().decode().replace(token,'[REDACTED]')
